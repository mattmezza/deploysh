...
deploy () {
    git checkout $LATEST_TAG && composer install && echo $LATEST_TAG > deploy/.current-version && notify_success
}

rollback () {
    git checkout $CURRENT_TAG && composer install && echo $CURRENT_TAG > deploy/.current-version && notify_failure
}
